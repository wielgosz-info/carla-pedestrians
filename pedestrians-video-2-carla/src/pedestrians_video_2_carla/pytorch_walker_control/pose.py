...
class P3dPose(Pose, torch.nn.Module):
    ...
    def __move_to_relative(self, changes: Tensor, prev_relative_rotations: Tensor) -> Tensor:
        """
        ...
        :param changes: (N, B, 3) tensor with the bone rotation changes.
        ...
        # TODO: "changes" is what will be generated by the model
        # think about what form would be best: euler angles? rotation matrices? quaternions? 6d rotation?
        # maybe allow to specify this when instantiating the P3dPose object?
        # Or: leave this to the model and always expect euler angles here?
        bs = changes.shape[0]
        change_matrix = euler_angles_to_matrix(changes, "XYZ")
        return torch.bmm(
            change_matrix.reshape((-1, 3, 3)),
    ...
    def forward(self, changes: Tensor, prev_relative_loc: Tensor, prev_relative_rot: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        """
        rot = self.__move_to_relative(changes, prev_relative_rot)
    ...
    def absolute(self):
        if self._last_abs_mod != self._last_rel_mod:
            ...
            absolute_pose = self.__tensors_to_pose(absolute_loc[0], absolute_rot[0])
    ...
    def move(self, rotations: Dict[str, carla.Rotation]):
        ...
        self.__relative_p3d_rotations = self.__move_to_relative(
            changes.unsqueeze(0), self.__relative_p3d_rotations.unsqueeze(0))[0]
        self._last_rel_mod = time.time_ns()
